...
public class PemHelper {
	...
	/**
	 * Decodes a PEM-encoded cryptographic object into the raw bytes of its
	 * ASN.1 encoding. Header/footer data and metadata info, e.g. Proc-Type, are
	 * ignored.
	 *
	 * @param pem
	 *            Bytes of PEM-encoded data to decode.
	 *
	...
	public static byte[] decode(final String pem) throws IOException {
		...
		final ByteBuffer buffer = ByteBuffer
				.allocateDirect(pem.length() * 3 / 4);
		String line;
		while ((line = reader.readLine()) != null) {
			if (line.startsWith(HEADER_BEGIN) || line.startsWith(FOOTER_END)
					|| line.startsWith(PROC_TYPE) || line.startsWith(DEK_INFO)
					|| line.trim().length() == 0) {
				continue;
			}
	...
	public static String encodeKey(final PrivateKey key, final char[] password,
			final SecureRandom random) throws IOException {
		if (password == null || password.length == 0) {
			...
			try {
				...
				writer.writeObject(key, KEY_ENCRYPTION_ALGORITHM, password,
						random);
			} finally {
				if (writer != null) {
					writer.close();
	 ...
	 *
	 * @param pemKey
	 *            PEM-encoded private key text to decode.
	 * @param password
	 *            Optional password that is used to decrypt private key using
	 *            DESEDE algorithm when specified.
	 *
	 * @return Private key.
	 *
	 * @throws IOException
	 *             On decoding error.
	 */
	public static PrivateKey decodeKey(final String pemKey,
	 ...
	 * data of PEM encoding. The determination is made using as little data from
	 * the given array as necessary to make a reasonable determination about
	 * encoding.
	 *
	 * @param data
	 *            Data to test for PEM encoding
	 *
	...
	public static boolean isPem(final byte[] data) {
		...
		try {
			...
			if (start.startsWith(HEADER_BEGIN) || start.startsWith(PROC_TYPE)) {
				...
				// Check all bytes in first line to make sure they are in the
				// range
			...
			throw new IllegalStateException(
					"YIKES! ASCII encoding not supported.");
		}
		return result;
	}
	 ...
	 *         valid characters for base64 encoding, false otherwise. The
	 *         padding character '=' is not considered valid since it may only
	 *         appear at the end of a base64 encoded value.
	 */
	public static boolean isBase64Char(final byte b) {
		return !(b < 47 || b > 122 || b > 57 && b < 65 || b > 90 && b < 97)
				|| b == 43;
	}
