...
public final class DERHelper {
	...
	public static byte[] asOctets(final DEREncodable e) {
		if (!(e instanceof DEROctetString)) {
			throw new IllegalArgumentException(
					"Argument must be DEROctetString.");
		}
	 ...
	 *
	 * @param data
	 *            ASN.1 encoded data as byte array.
	 * @param discardWrapper
	 *            Set to true to decode the octets of a DER octet string as DER
	 *            encoded data, thereby discarding the wrapping DER octet
	 *            string, false otherwise. Has no effect on other types of
	 *            DER-encoded data.
	 *
	 * @return DER object.
	 *
	...
	public static DERObject toDERObject(final byte[] data,
		...
		try {
			...
			if (discardWrapper && o instanceof ASN1OctetString) {
				o = new ASN1InputStream(((ASN1OctetString) o).getOctets())
						.readObject();
			}
			return o;
		} finally {
	 ...
	 *
	 * @param items
	 *            One or more DER encodable items.
	 *
