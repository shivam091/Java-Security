 ...
 * 
 */
public class OpenSSLEncryptionScheme extends
	 ...
	 *            cipher is expected to be initialized with data required for
	 *            use except, of course, the private key which will be generated
	 *            from a password.
	 * @param salt
	 *            Salt data for key generation function.
	 * @param keyBitLength
	 *            Size of derived keys in bits.
	 */
	public OpenSSLEncryptionScheme(final SymmetricAlgorithm alg,
			final byte[] salt, final int keyBitLength) {
		setCipher(alg);
	 ...
	 *
	 * @param algId
	 *            Identifier describing the cipher used for key encryption. The
	 *            form of the identifier follows the following convention:<br>
	 *            $ALGORITHM-$MODE
	 * @param iv
	...
	public OpenSSLEncryptionScheme(final String algId, final byte[] iv) {
		...
		if (algId.startsWith("DES-EDE3")) {
			...
			if (algId.startsWith("AES-128")) {
				...
				throw new IllegalArgumentException("Unknown AES cipher "
						+ algId);
			}
		} else {
