...
public class Rijndael implements Cloneable {
	...
	private int[] rijndaelRCon = { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40,
			0x80, 0x1b, 0x36 };
	private int[][] rijndaelMixColumnsMatrix = { { 0x02, 0x03, 0x01, 0x01 },
	...
	public Rijndael(int[] key) throws Exception {
		if (key.length != 16) {
			// Currently only supports 128-bit keys. Will be expanded in a
			// future version
	...
	public int[] encrypt(int[] plaintext) throws Exception {
		if (plaintext.length != 16) {
			...
			for (int i = 1; i < 10; i++) {
				ctMatrix = rijndaelAddRoundKey(
						rijndaelMixColumns(rijndaelShiftRows(rijndaelSubints(ctMatrix))),
						rijndaelRKeys[i]);
			}
			ctMatrix = rijndaelAddRoundKey(
					rijndaelShiftRows(rijndaelSubints(ctMatrix)),
					rijndaelRKeys[10]);
			for (int i = 0; i < 4; i++) {
	...
	public int[] decrypt(int[] ciphertext) throws Exception {
		if (ciphertext.length != 16) {
			...
			for (int i = 9; i > 0; i--) {
				ptMatrix = rijndaelInverseMixColumns(rijndaelAddRoundKey(
						rijndaelInverseSubints(rijndaelInverseShiftRows(ptMatrix)),
						rijndaelRKeys[i]));
			}
			ptMatrix = rijndaelAddRoundKey(
					rijndaelInverseSubints(rijndaelInverseShiftRows(ptMatrix)),
					rijndaelRKeys[0]);
			for (int i = 0; i < 4; i++) {
	...
	private int[][] rijndaelMixColumns(int[][] B) throws Exception {
		if (B.length != 4 || B[0].length != 4) {
			...
			for (int i = 0; i < 4; i++) {
				for (int j = 0; j < 4; j++) {
					for (int k = 0; k < 4; k++) {
						mixed[j][i] ^= rijndaelGFMult(
								rijndaelMixColumnsMatrix[i][k], B[j][k]);
					}
				}
			}
			return mixed;
		...
		if (B.length != 4 || B[0].length != 4) {
			...
			for (int i = 0; i < 4; i++) {
				for (int j = 0; j < 4; j++) {
					for (int k = 0; k < 4; k++) {
						mixed[j][i] ^= rijndaelGFMult(
								rijndaelInverseMixColumnsMatrix[i][k], B[j][k]);
					}
				}
			}
			return mixed;
	...
	private int[][] rijndaelAddRoundKey(int[][] B, int[][] roundKey)
			throws Exception {
		if (B.length != 4 || B[0].length != 4) {
			throw new Exception("B must be 4x4 ints");
