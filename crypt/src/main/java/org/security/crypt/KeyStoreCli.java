...
public class KeyStoreCli extends AbstractCli {
	...
	protected void initOptions() {
		...
		final Option type = new Option(OPT_TYPE, true,
				"keystore type, e.g. BKS (default), JKS");
		...
		final Option key = new Option(OPT_KEY, true,
				"DER-encoded PKCS#8 or PEM-encoded SSLeay private key; "
						+ "encoding determined by file extension (der|pem)");
	...
	protected void list(final CommandLine line) throws Exception {
		...
		while (aliases.hasMoreElements()) {
			...
			System.out
					.println("Creation date: " + store.getCreationDate(alias));
	...
	protected void doImport(final CommandLine line) throws Exception {
		...
		if (line.hasOption(OPT_KEY)) {
			...
			final char[] passChars = line.getOptionValue(OPT_PASS)
					.toCharArray();
			final PrivateKey key = CryptReader.readPrivateKey(keyFile);
			final Certificate[] chain = CryptReader
					.readCertificateChain(certFile);
			System.err.println("Read certificate chain of length "
					+ chain.length + ":");
			for (int i = 0; i < chain.length; i++) {
				System.out.println("===== Certificate [" + i + "] =====");
				printCertificate(chain[i]);
	...
	protected void doExport(final CommandLine line) throws Exception {
		...
		if (line.hasOption(OPT_KEY)) {
			final File keyFile = new File(line.getOptionValue(OPT_KEY));
			final PrivateKey key = (PrivateKey) store.getKey(alias, line
					.getOptionValue(OPT_PASS).toCharArray());
		...
		if (!wroteData) {
			System.err
					.println("No data was written because neither -cert nor -key was specified.");
		}
	}
	...
	protected void printCertificate(final Certificate cert) throws Exception {
		if (cert instanceof X509Certificate) {
			...
			System.out.println("Serial: "
					+ hexConv.fromBytes(xCert.getSerialNumber().toByteArray()));
			System.out.println("Valid not before: " + xCert.getNotBefore());
			System.out.println("Valid not after: " + xCert.getNotAfter());
			System.out.println("MD5 fingerprint: "
					+ md5.digest(encodedCert, hexConv));
			System.out.println("SHA1 fingerprint: "
					+ sha1.digest(encodedCert, hexConv));
		} else {
