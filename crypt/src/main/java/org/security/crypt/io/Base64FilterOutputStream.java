 ...
 * 
 */
public class Base64FilterOutputStream extends
	...
	/**
	 * Write out buffer size must be multiple of 3 to accommodate 24-bit groups
	 * of base 64 encoding.
	 */
	private static final int BUFFER_SIZE = 1536;

	/** Buffers raw bytes written out to facilite writing on 24-bit boundaries. */
	private final DirectByteArrayOutputStream writeBuffer = new DirectByteArrayOutputStream(
			BUFFER_SIZE);
	...
	/**
	 * 0-based column in which last flush operation ended for line-wrapped
	 * output.
	 */
	 ...
	 *            Output stream to wrap.
	 * @param charsPerLine
	 *            Number of characters per line of encoded output. Must be one
	 *            of {@link #LINE_LENGTH_64}, {@link #LINE_LENGTH_76}, or 0 to
	...
	public Base64FilterOutputStream(final OutputStream out,
		...
		if (charsPerLine == 0) {
			lineBuffer = null;
		} else if (charsPerLine == LINE_LENGTH_64
				|| charsPerLine == LINE_LENGTH_76) {
			lineBuffer = new DirectByteArrayOutputStream(BUFFER_SIZE
					* CHARS_PER_GROUP / BYTES_PER_GROUP);
		} else {
	...
	protected void flushToStream() throws IOException {
		if (lineLength == 0) {
			...
			lineBuffer.reset();
			encoder.encode(writeBuffer.getBuffer(), 0, writeBuffer.size(),
					lineBuffer);
