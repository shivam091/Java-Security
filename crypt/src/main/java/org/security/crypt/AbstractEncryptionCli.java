...
import org.security.crypt.io.Base64FilterOutputStream;
import org.security.crypt.io.HexFilterInputStream;
import org.security.crypt.io.HexFilterOutputStream;
import org.security.crypt.io.TeePrintStream;
...
public abstract class AbstractEncryptionCli extends AbstractCli {
	...
	protected void initOptions() {
		...
		final Option outfile = new Option(OPT_OUTFILE, true,
	 ...
	 * @param in
	 *            Input stream containing plaintext.
	 * @param out
	 *            Output stream containing ciphertext.
	 *
	 * @throws Exception
	 *             On encryption errors.
	 */
	protected void encrypt(final EncryptionAlgorithm alg, final InputStream in,
	...
	/**
	 * Decrypt the given ciphertext input stream into the output stream using
	 * the given encryption cipher algorithm.
	 *
	 * @param alg
	 *            Encryption cipher algorithm.
	 * @param in
	 *            Input stream containing ciphertext.
	 * @param out
	 *            Output stream containing plaintext.
	 *
	 * @throws Exception
	 *             On encryption errors.
	 */
	protected void decrypt(final EncryptionAlgorithm alg, final InputStream in,
			final OutputStream out) throws Exception {
		try {
	...
	/**
	 * Get an output stream containing data to be encrypted or decrypted based
	 * on CLI arguments.
	 *
	 * @param line
	 *            Parsed command line arguments container.
	...
	protected OutputStream getOutputStream(final CommandLine line)
		...
		if (line.hasOption(OPT_OUTFILE)) {
			...
			if (line.hasOption(OPT_TAIL)) {
				out = new TeePrintStream(new BufferedOutputStream(
						new FileOutputStream(file)), System.out);
			} else {
