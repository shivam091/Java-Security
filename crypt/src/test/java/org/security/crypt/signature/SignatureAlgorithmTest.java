...
import org.security.crypt.asymmetric.PublicKeyUtils;
import org.security.crypt.digest.MD5;
import org.security.crypt.digest.SHA256;
import org.security.crypt.digest.SHA512;
import org.security.crypt.digest.Tiger;
import org.security.crypt.digest.Whirlpool;
...
public class SignatureAlgorithmTest {
	...
	public Object[][] createTestData() throws Exception {
		return new Object[][] {
				{ new DSASignature(), PublicKeyUtils.generate("DSA", 1024),
						null, },
				{ new DSASignature(), PublicKeyUtils.generate("DSA", 512),
						new Base64Converter(), },
				...
				{ new DSASignature(new Tiger()),
						PublicKeyUtils.generate("DSA", 1024),
						new HexConverter(), },
				{ new RSASignature(), PublicKeyUtils.generate("RSA", 1024),
						null, },
				{ new RSASignature(), PublicKeyUtils.generate("RSA", 512),
						new Base64Converter(), },
				...
				{ new RSASignature(new Whirlpool()),
						PublicKeyUtils.generate("RSA", 1048),
						new HexConverter(), },
				{ new ECDSASignature(), PublicKeyUtils.generate("EC", 256),
						null, },
				{ new ECDSASignature(new SHA256()),
						PublicKeyUtils.generate("EC", 384), new HexConverter(), }, };
	}
	...
	public Object[][] createTestDataForClone() throws Exception {
		...
		return new Object[][] { { rsa }, { ecdsa }, };
	}

	/**
	 * @param signature
	 *            A crypto signature algorithm to test.
	 * @param keys
	 *            Public/private key pair used for signing.
	 * @param converter
	 *            Converter used to convert sig bytes to String.
	 *
	 * @throws Exception
	 *             On test failure.
	 */
	@Test(groups = { "functest", "signature" }, dataProvider = "testdata")
	public void testSignVerify(final SignatureAlgorithm signature,
			final KeyPair keys, final Converter converter) throws Exception {
		logger.info("Testing signature algorithm {} with converter {}",
				signature, converter);
		...
		if (converter == null) {
			...
			AssertJUnit.assertTrue(signature.verify(CLEARTEXT.getBytes(),
					signedBytes));
		} else {
			final String sig = signature.sign(CLEARTEXT.getBytes(), converter);
			signature.setVerifyKey(keys.getPublic());
			signature.initVerify();
			AssertJUnit.assertTrue(signature.verify(CLEARTEXT.getBytes(), sig,
					converter));
		}
	}
	 ...
	 */
	@Test(groups = { "functest", "signature" }, dataProvider = "testdata")
	public void testRandomizedSignVerify(final SignatureAlgorithm signature,
			final KeyPair keys, final Converter converter) throws Exception {
		logger.info(
	 ...
	 */
	@Test(groups = { "functest", "signature" }, dataProvider = "testdata")
	public void testSignVerifyOnStream(final SignatureAlgorithm signature,
			final KeyPair keys, final Converter converter) throws Exception {
		logger.info(
	...
	/**
	 * @param algorithm
	 *            Signature algorithm to clone and test.
	 *
	 * @throws Exception
	 *             On test failure.
	 */
	@Test(groups = { "functest", "signature" }, dataProvider = "testclone")
	public void testClone(final SignatureAlgorithm algorithm) throws Exception {
		final SignatureAlgorithm clone = (SignatureAlgorithm) algorithm.clone();
		algorithm.initSign();
		clone.initSign();
		final byte[] cleartext = "Able was I ere I saw elba".getBytes();
		AssertJUnit.assertArrayEquals(algorithm.sign(cleartext),
				clone.sign(cleartext));
	}
}