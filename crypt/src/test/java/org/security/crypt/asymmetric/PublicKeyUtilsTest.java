...
public class PublicKeyUtilsTest {
	 ...
	 * @param keys
	 *            Valid key pair.
	 * @param expectedPubLen
	 *            Expected public key length.
	 * @param expectedPrivLen
	 *            Expected private key length.
	 *
	 * @throws Exception
	 *             On errors.
	 */
	@Test(groups = { "functest", "asymmetric" }, dataProvider = "keypairs")
	public void testLength(final KeyPair keys, final int expectedPubLen,
			final int expectedPrivLen) throws Exception {
		AssertJUnit.assertEquals(expectedPubLen,
				PublicKeyUtils.length(keys.getPublic()));
		AssertJUnit.assertEquals(expectedPrivLen,
				PublicKeyUtils.length(keys.getPrivate()));
	}

	/**
	 * Unit test for
	 * {@link PublicKeyUtils#isKeyPair(java.security.PublicKey, java.security.PrivateKey)}
	 * .
	 *
	 * @param pubKey
	 *            Public key.
	 ...
	 *            Private key.
	 * @param expected
	 *            Expected result of keypair test.
	 *
	 * @throws Exception
	 *             On errors.
	 */
	@Test(groups = { "functest", "asymmetric" }, dataProvider = "keys")
	public void testIsKeyPair(final PublicKey pubKey, final PrivateKey privKey,
