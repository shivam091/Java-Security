...
import org.security.crypt.x509.types.KeyUsageBits;
...
public class ExtensionReaderTest {
	...
	public Object[][] createCertificateTestData() throws Exception {
		...
		final Map<ExtensionType, Object> extMap1 = new HashMap<ExtensionType, Object>();
		extMap1.put(ExtensionType.SubjectAlternativeName, new GeneralNameList(
				new GeneralName[] { new GeneralName("serac@vt.edu",
		...
		final PolicyInformation[] policies1 = new PolicyInformation[] {
				...
				new PolicyInformation("1.3.6.1.4.1.6760.5.2.2.4.1",
						new PolicyQualifierInfo[] { new PolicyQualifierInfo(
								"http://www.pki.vt.edu/vtc1sca/cps/"), }),
				new PolicyInformation("1.3.6.1.4.1.6760.5.2.2.3.1"), };
		extMap1.put(ExtensionType.CertificatePolicies,
				new PolicyInformationList(policies1));
		extMap1.put(ExtensionType.SubjectKeyIdentifier, new KeyIdentifier(
		...
		extMap1.put(ExtensionType.KeyUsage, new KeyUsage(new KeyUsageBits[] {
				KeyUsageBits.DigitalSignature, KeyUsageBits.NonRepudiation, }));
		extMap1.put(ExtensionType.ExtendedKeyUsage,
				new KeyPurposeIdList(new KeyPurposeId[] {
						KeyPurposeId.EmailProtection, KeyPurposeId.ClientAuth,
						KeyPurposeId.SmartCardLogin, }));
		extMap1.put(
				ExtensionType.CRLDistributionPoints,
				new DistributionPointList(
						Collections.singletonList(new DistributionPoint(
								new GeneralNameList(
										Collections
												.singletonList(new GeneralName(
		...
		final Map<ExtensionType, Object> extMap2 = new HashMap<ExtensionType, Object>();
		...
		final Map<ExtensionType, Object> extMap3 = new HashMap<ExtensionType, Object>();
		extMap3.put(ExtensionType.BasicConstraints, new BasicConstraints(false));
		extMap3.put(ExtensionType.SubjectKeyIdentifier, new KeyIdentifier(
				"31:AE:F1:7C:98:67:E9:1F:19:69:A2:A7:84:1E:67:5C:AA:C3:6B:75"));
		extMap3.put(ExtensionType.KeyUsage, new KeyUsage(new KeyUsageBits[] {
				KeyUsageBits.DigitalSignature, KeyUsageBits.KeyEncipherment, }));
		extMap3.put(
				ExtensionType.CertificatePolicies,
				new PolicyInformationList(
						Collections
								.singletonList(new PolicyInformation(
										"2.16.840.1.113733.1.7.23.6",
										new PolicyQualifierInfo[] { new PolicyQualifierInfo(
												"https://www.verisign.com/rpa"), }))));
		extMap3.put(ExtensionType.ExtendedKeyUsage, new KeyPurposeIdList(
				new KeyPurposeId[] { KeyPurposeId.ClientAuth,
		...
		return new Object[][] { { testCert1, extMap1 }, { testCert2, extMap2 },
				{ testCert3, extMap3 }, };
	}
	 ...
	 * @param expectedExtensionMap
	 *            Expected map of extension types to extension data that should
	 *            be produced from reading the extended attributes of the given
	 *            certificate.
	 *
	 * @throws Exception
	 *             On test failure.
	...
	public void testReadAll(final File certFile,
		...
		/*
		 * logger.info( "DER dump of {}:\n\n{}", certFile,
		 * ASN1Dump.dumpAsString( new
		 * ASN1InputStream(cert.getEncoded()).readObject()));
		 */
		...
		for (ExtensionType type : actualExtensionMap.keySet()) {
			sb.append('\t').append(type).append('=')
					.append(actualExtensionMap.get(type)).append('\n');
		}
		logger.info("Attributes found:\n{}", sb.toString());

		AssertJUnit.assertEquals(expectedExtensionMap.size(),
				actualExtensionMap.size());
		for (ExtensionType type : expectedExtensionMap.keySet()) {
			AssertJUnit.assertEquals(expectedExtensionMap.get(type),
