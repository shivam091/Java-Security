...
public class RDNSequenceIteratorTest {
	...
	public Object[][] createRDNTestData() throws Exception {
		...
		final RelativeDistinguishedName[] expected1 = new RelativeDistinguishedName[] {
				new RelativeDistinguishedName(new AttributeTypeAndValue(
						AttributeType.CountryName, "US")),
				new RelativeDistinguishedName(new AttributeTypeAndValue(
						AttributeType.DomainComponent, "vt")),
				new RelativeDistinguishedName(new AttributeTypeAndValue(
						AttributeType.DomainComponent, "edu")),
				new RelativeDistinguishedName(new AttributeTypeAndValue(
						AttributeType.OrganizationName,
						"Virginia Polytechnic Institute and State University")),
				new RelativeDistinguishedName(new AttributeTypeAndValue(
						AttributeType.CommonName, "Marvin S Addison")),
				new RelativeDistinguishedName(new AttributeTypeAndValue(
						AttributeType.UserId, "1145718")), };

		return new Object[][] { { testCert1, expected1 }, };
	}

	/**
	 * @param certFile
	 *            File containing X.509 certificate data.
	 * @param expectedSubjectRDNs
	 *            Expected array of relative distinguished names that should be
	 *            collected upon iterating over the subject DN of the cert.
	 *
	 * @throws Exception
	 *             On test failure.
	 */
	@Test(groups = { "functest", "x509" }, dataProvider = "rdndata")
	public void testIterator(final File certFile,
		...
		final List<RelativeDistinguishedName> actual = new ArrayList<RelativeDistinguishedName>();
